...
export default function SubContractorSignInForm() {
	...
	const onSubmit = (data: UserLoginType) => {
		clearCookies();
		loginMutation(
			{ ...data, userType: ROLES.SUB_CONTRACTOR },
			{
				onSuccess: (res) => {
					const { token } = res;
					setLoginCookies({ token, userType: ROLES.SUB_CONTRACTOR });
					openSuccessToast("Logged in successfully");
					const redirectRoute = redirectUser(ROLES.SUB_CONTRACTOR);
		...
		>
			...
			<div className="relative z-10 flex w-full max-w-md flex-col px-6 py-6">
				<div className="my-6 text-center">
					<p className="my-2 text-[20px] font-normal text-white">Sub-Contractor Admin Login</p>
				</div>
