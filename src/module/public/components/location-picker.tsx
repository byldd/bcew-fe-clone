...
type LocationPickerProps = {
	onSelectedChange?: (location: SelectedLocation | null) => void;
...
const LocationPicker = ({
	...
	const [isResolvingSpeed, setIsResolvingSpeed] = useState(false);
	...
	const setLocation = useCallback(
		(lat: number, lng: number, panTo = true) => {
			...
			setSelected({ lat, lng, address: "", googleMapsLink: buildGoogleMapsLink(lat, lng), speedLimit: null });
			...
			if (onResolveSpeedLimit) {
				setIsResolvingSpeed(true);
				onResolveSpeedLimit(lat, lng)
					.then((speedLimit) => {
						setSelected((prev) => (prev && prev.lat === lat && prev.lng === lng ? { ...prev, speedLimit } : prev));
					})
					.finally(() => setIsResolvingSpeed(false));
			}
		},
		[onResolveSpeedLimit]
	);
	...
	useEffect(() => {
		if (seededRef.current || !isLoaded || !initialLocation) return;
		seededRef.current = true;
		setLocation(initialLocation.lat, initialLocation.lng);
	}, [isLoaded, initialLocation, setLocation]);
	...
	if (!isLoaded) {
		return (
			<div className="flex h-[300px] w-full items-center justify-center rounded-2xl bg-brand-bgLightgrey">
				<Spinner />
			</div>
		);
	}
		...
		<div className="space-y-3">
			<div className="flex gap-3">
				...
				<Button
					...
					className="h-11 w-11 shrink-0 rounded-[10px] bg-brand-dark p-0 text-white hover:bg-gray-800"
				>
					<Crosshair className={`h-4 w-4 ${isLocating ? "animate-pulse" : ""}`} />
				</Button>
			</div>

			<div className="h-[260px] w-full overflow-hidden rounded-2xl ring-1 ring-gray-900/5">
				<GoogleMap
			...
			{selected && (
				<div className="space-y-4 rounded-2xl bg-brand-bgLightgrey p-4">
					<div className="flex items-start gap-3">
						<MapPin className="text-brand-blue mt-0.5 h-5 w-5 shrink-0" />
						<div className="min-w-0">
							<p className="text-sm font-semibold text-brand-dark">Selected Location</p>
