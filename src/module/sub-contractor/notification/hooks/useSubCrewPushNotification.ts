...
import {
	useSubCrewSubscribeToPushNotification,
	useSubCrewUnsubscribeFromPushNotification,
} from "./useSubContractorNotification";
...
export function useSubCrewPushNotification() {
	const tschedule = useTypedTranslations(NAMESPACE.SCHEDULE);
	const [isSupported, setIsSupported] = useState(false);
	const [isSubscribed, setIsSubscribed] = useState(false);
	const { subcontractorCrew } = useAuthStore((state) => state);

	const { mutate: subscribeToPushNotification, isPending: isSubscribing } = useSubCrewSubscribeToPushNotification();
	const { mutate: unsubscribeFromPushNotification, isPending: isUnsubscribing } =
		useSubCrewUnsubscribeFromPushNotification();
	...
	const checkSubscriptionStatus = useCallback(async () => {
		try {
			...
			setIsSubscribed(
				!!sub &&
					!!subcontractorCrew?.subCrewPushSubscriptions?.some((subscription) => subscription.endpoint === sub?.endpoint)
			);
		} catch (error) {
			console.error("Error checking subscription status:", error);
		}
	}, [subcontractorCrew?.subCrewPushSubscriptions]);
